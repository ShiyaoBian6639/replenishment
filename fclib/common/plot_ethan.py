...
def format_func(value, tick_number):
    return (datetime.date(2015,1,1)+ timedelta(days = int(value) * 7 )).strftime('%d %b')


def plot_predictions_with_history_6_months(
    predictions,
    history,
    grain1_unique_vals,
    grain2_unique_vals,
        ...
        for col in row:
            if sample_id < len(sample_grain_combinations):
                ...
                history_sub = history.loc[
                    ...
                    & (history[time_col_name] <= min_timestep + 500) #for six months
                ]
                predictions_sub = predictions.loc[
                    ...
                    & (predictions[time_col_name] <= min_timestep+500) #fpr six months
                ...
                col.xaxis.set_major_formatter(plt.FuncFormatter(format_func))
...
def plot_predictions_with_history_3_months(
    predictions,
    history,
    grain1_unique_vals,
    grain2_unique_vals,
        ...
        for col in row:
            if sample_id < len(sample_grain_combinations):
                ...
                history_sub = history.loc[
                    ...
                    & (history[time_col_name] <= max_timestep)
                    & (history[time_col_name] >= min_timestep)
                    & (history[time_col_name] <= min_timestep + 36) #for three months
                ]
                predictions_sub = predictions.loc[
                    (predictions[grain1_name] == grain1_id)
                    & (predictions[grain2_name] == grain2_id)
                    & (predictions[time_col_name] >= min_timestep)
                    & (predictions[time_col_name] <= min_timestep + 36) #for three months

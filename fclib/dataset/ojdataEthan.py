...
from fclib.fclib.common.utils import git_repo_path
from fclib.fclib.feature_engineering.feature_utils import df_from_cartesian_product
...
def download_ojdata(dest_dir="."):
    ...
    if not data_exists:
        ...
        repo_path = r'C:\Users\ethansu\Desktop\Orange juice'
...
def _gen_split_indices(n_splits=12, horizon=2, gap=2, first_week=40, last_week=275):
...
def split_train_test(name, data_dir, n_splits=1, horizon=2, gap=2, first_week=40, last_week=275, write_csv=False):
    ...
    sales = pd.read_csv(os.path.join(data_dir, name), index_col=0)
    print(sales)
    sales.sort_values(by=['store', 'brand'])
